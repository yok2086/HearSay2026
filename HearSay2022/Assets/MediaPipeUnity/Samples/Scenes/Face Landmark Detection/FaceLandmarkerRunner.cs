...
namespace Mediapipe.Unity.Sample.FaceLandmarkDetection
{
  public class FaceLandmarkerRunner : VisionTaskApiRunner<FaceLandmarker>
  {
    ...
    [Header("HearSay Speaker Test")]
    [SerializeField] private HearSay.SpeakerMouthTest _speakerMouthTest;
    ...
    protected override IEnumerator Run()
    {
      ...
      var result =
        FaceLandmarkerResult.Alloc(options.numFaces);
      ...
      while (true)
      {
        ...
        if (!_textureFramePool.TryGetTextureFrame(
        ...
        switch (taskApi.runningMode)
        {
          case Tasks.Vision.Core.RunningMode.IMAGE:
            ...
            if (taskApi.TryDetect(
              ...
              ref result))
            {
              _faceLandmarkerResultAnnotationController.DrawNow(
                result
              );
    ...
    private void SendResultToSpeakerTest(
      FaceLandmarkerResult result)
    {
      if (_speakerMouthTest != null)
      {
        _speakerMouthTest.ProcessFaceResult(
          result
        );
      }
    }
